...
int main(int argc, char** argv) {
    if (argc > 1) {
        ...
        g.triangleCountV4(1, &duration, "cilk");
        ...
        if (argc > 2 && strcmp(argv[2], "--debug") == 0) {
            ...
            printf("V4 cilk and V3 c3 differed in %d elements\n", c3_wrong_count);
        } else if (argc > 2 && strcmp(argv[2], "--test") == 0) {
            FILE *fp = fopen("v4_cilk_bmark.txt", "w+");
            for (int i = 1; i <= 16; i++) {
                g.triangleCountV4(1, &duration, "cilk", i); 
    ...
    struct dirent *dir;
    if (d) {
        FILE *fp = fopen("v4_cilk_times.txt", "w+");

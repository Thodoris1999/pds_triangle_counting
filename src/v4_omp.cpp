...
int main(int argc, char** argv) {
    if (argc > 1) {
        ...
        g.triangleCountV4(1, &duration, "omp");
        ...
        if (argc > 2 && strcmp(argv[2], "--debug") == 0) {
            ...
            printf("V4 omp and V3 c3 differed in %d elements\n", c3_wrong_count);
        } else if (argc > 2 && strcmp(argv[2], "--test") == 0) {
            FILE *fp = fopen("v4_omp_bmark.txt", "w+");
            for (int i = 1; i <= 16; i++) {
                g.triangleCountV4(1, &duration, "omp", i); 
    ...
    struct dirent *dir;
    if (d) {
        FILE *fp = fopen("v4_omp_times.txt", "w+");
